...
| `breakfast` | 🧪 uses the real parts as ingredients (break + fast) — compound-word rule, not yet verified live |
| `bat` | Card teaches one kid-most-common sense; the other sense appears only as a 🎯 easter egg — polysemy rule, not yet verified live |
| `猫` (or `cat`) | Low-age floor: cuter/sillier, near-zero etymology, heavy拟声 — auto-scale bottom end, not yet verified live |
